...
public class ViewPagerAdapter extends RecyclerView.Adapter<ViewPagerAdapter.ViewHolder> implements SharedConstants
{
    ...
    public void onBindViewHolder(ViewHolder holder, int position) {
		...
		switch(page){
			...
			case 3:
				...
				timeDelay=500;
				pageNumber=3;
				anim(textViewPage3,timeDelay,pageNumber);
				to_another_page(textViewPage3,0,2000);
				break;

				
			case 4:
				 TextView textViewPage4=holder.textViewPage_4;
				 to_another_page(textViewPage4,5,1500);
				 break;
			...
			case 6:
				...
				//timeDelay=500;
				//pageNumber=6;
				//anim(textViewPage6,timeDelay,pageNumber);
				to_another_page(textViewPage6,7,3000);
				break;
				 

			case 7:
				TextView textViewPage7=holder.textViewPage_7;
				to_another_page(textViewPage7,0,5000);
				break;
		}
	...
	/*public void anim2(final TextView textView, int timeDelay, int _page){
	...
	public class myCh implements OnChronometerTickListener
	{
		...
		public void onChronometerTick(final Chronometer cArg) {
			...
			/*if(time > 5*60*1000) {//если прошло больше 5 минут
			 ...
			 chronometer.setTextColor(Color.parseColor("#bdbdbd"));
			 }*/
			//внутреннее ветвление			



			/*//отправляемся на другой экран
			 //if(time > 1*60*1000) {//если прошло уже больше 1 минут
			 if(ss.equals("00")|ss.equals("20")|ss.equals("40")){//если секунды 00 
			...
			/*if(seconds.equals("00")){
				...
				pageRegularly=1;//указываем первую последовательность разметок
			}
			 
			if(seconds.equals("30")){
				//MainActivity. viewPagerAdapter. notifyDataSetChanged();
				pageRegularly=2;//указываем вторую последовательность разметок
			}*/
    ...
    /*//метод принимает на вход начальное время в мс
	 public void startChronometer(long initialTime){
	 //найдем разность, чтобы отобразить прошедшее время назад
	 chronometer.setBase(SystemClock.elapsedRealtime()-initialTime);//вот, так покажет с разностью времени последней фиксации (плюс будет тикать время, прибавляться)
	 //chronometer.setBase(SystemClock.elapsedRealtime());//а так, показал бы от нуля. И тикало/прибавлялось бы, от нуля
	 //запустим чтобы тикало:
	...
	public class ViewHolder extends RecyclerView.ViewHolder {
		...
		ViewHolder(View itemView)
		{
			...
			//четвертая стр
			textViewPage_3=itemView. findViewById(R.id.textViewPage3);
